...
public class Recommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		// Step 2, count all keywords for all the favorite items by this user, sort by count
		// {"software engineer": 6, "backend": 4, "san francisco": 3, "remote": 1}
		Map<String, Integer> allKeywords = new HashMap<>();
		// counting 
		for (String itemId : favoritedItemIds) {
			// get each item's keywords from DB connection
			Set<String> keywords = connection.getKeywords(itemId);
			for (String keyword : keywords) {
				// count the each keyword's occurrence
				allKeywords.put(keyword, allKeywords.getOrDefault(keyword, 0) + 1);
			}
		}
		connection.close();
		// data processing: convert the map's entry to list, sort the keyword list
		...
		// Step 3, search based on keywords, filter out favorite items and visited items
		...
		for (Entry<String, Integer> keyword : keywordList) {
			...
			for (Item item : items) {
				// new recommend should not be liked before
